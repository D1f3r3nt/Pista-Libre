...
fun SignClubScreen(
    ...
    Box(
        Modifier
            ...
            FooterRegistroClub(Modifier.align(Alignment.BottomCenter), navigationController)
        }
}

@Composable
fun FooterRegistroClub(modifier: Modifier, navController: NavHostController) {
    Column(modifier = modifier.fillMaxWidth()) {
        GoToRegisterPlayer(navController)
...
@Composable
fun GoToRegisterPlayer(navController: NavHostController){
    val colorVerde = colorResource(id = R.color.verdeApp)
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center){
        Text(text = "Registrarme como jugador", Modifier.padding(horizontal = 8.dp).clickable { navController.navigate("registroJugador") }, fontSize = 12.sp, color = colorVerde)
    }
}
...
@Composable
fun BodyRegistroClub(modifier: Modifier, signClubViewModel: SignClubViewModel) {
    var clubName by rememberSaveable { mutableStateOf("") }
    var directionClub by rememberSaveable { mutableStateOf("") }
    val email: String by signClubViewModel.email.observeAsState(initial = "")
    val password: String by signClubViewModel.password.observeAsState(initial = "")
    var repeatpasswordclub by rememberSaveable { mutableStateOf("") }
    val signClubEnable by signClubViewModel.isSignClubEnable.observeAsState(initial = false)

    Column(modifier = modifier.width(300.dp)) {
        ClubName(clubName) { clubName = it }
        Spacer(modifier = Modifier.size(16.dp))
        DirectionClub(directionClub) { directionClub = it }
        Spacer(modifier = Modifier.size(16.dp))
...
fun SingUpClubButton(signClubEnable: Boolean, signClubViewModel: SignClubViewModel) {
    val colorVerde = colorResource(id = R.color.verdeApp)
    Button(onClick = { signClubViewModel.onSignUpClubSelected()},
...
fun RepeatPasswordClub(repeatpasswordclub:String, onTextChanged: (String) -> Unit) {
    var passwordVisibility by remember { mutableStateOf(false) }
    TextField(value = repeatpasswordclub,
        onValueChange = { onTextChanged(it) },
        modifier = Modifier.fillMaxWidth(),
...
@Composable
fun HeaderRegistroClub(modifier: Modifier) {
    Column(modifier = modifier) {
        TextRegistroClub(modifier = modifier)
    }
}

@Composable
fun TextRegistroClub(modifier: Modifier) {
    val colorVerde = colorResource(id = R.color.verdeApp)
    Column(
        Modifier
            .fillMaxWidth()
