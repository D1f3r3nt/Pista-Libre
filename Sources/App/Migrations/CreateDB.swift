...
struct CreateDB: AsyncMigration {
    func prepare(on database: Database) async throws {
        ...
        try await database.schema("court")
            .field("id", .int, .identifier(auto: true))
            .field("club", .int, .required, .references("club", "id"))
        ...
        try await database.schema("booking")
            .field("id", .int, .identifier(auto: true))
            .field("court", .int, .required, .references("court", "id"))
            .field("date", .string, .required)
            .field("p1", .int, .required, .references("user", "id"))
            .field("p2", .int, .references("user", "id"))
            .field("p3", .int, .references("user", "id"))
            .field("p4", .int, .references("user", "id"))
